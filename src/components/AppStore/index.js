...
class AppStore extends React.Component {
  state = {tabId: tabsList[0].tabId, search: ''}

  updateTabItem = tab => {
    this.setState({tabId: tab})
  }

  searchInput = event => this.setState({search: event.target.value})

  render() {
    const {tabId, search} = this.state
    const filteredAppsList = appsList.filter(obj => obj.category === tabId)
    const searchFilterList = filteredAppsList.filter(obj =>
      obj.appName.toLowerCase().includes(search.toLowerCase()),
    )
    return (
        ...
        <ul className="tabs">
          {tabsList.map(obj => (
            <TabItem
              ...
              key={obj.id}
              isActive={obj.tabId === tabId}
            />
          ))}
        </ul>

        <ul className="apps-container">
          {searchFilterList.map(obj => (
            <AppItem appList={obj} key={obj.appId} />
          ))}
        </ul>
      </div>
    )
